...
public class PunchcardTotalCmd implements Command {
    ...
    public CommandEnum type() {
        return CommandEnum.punchcard_total;
    }

    @Override
    public ApiResponse execute(String commandRequest, LoginInfo loginInfo) {

        ApiResponse dateChResult = RegexUtil.extractDateCh(commandRequest, type());
        if(!dateChResult.isSuccess()){
            return dateChResult;
        }
        String dateCh = (String)dateChResult.getData();

        ApiResponse startTimeResult = RegexUtil.extractDateFromCh(commandRequest, type());
